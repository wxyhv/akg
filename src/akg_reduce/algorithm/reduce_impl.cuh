...
__inline__ __device__ void AllReduce(ReduceOp op,      // the operator
  ...
  HalvedReduce1D<ReduceOp, blockSize_x>(shared_array, &acc, op);
  if (threadIdx.x == 0) {
...
__inline__ __device__ void ReduceDirectionX(ReduceOp op,      // the operator
  ...
  HalvedReduce2DX<ReduceOp, blockSize_x>(shared_array, &acc, op);
...
__inline__ __device__ void ReduceDirectionY(ReduceOp op,      // the operator
  ...
  HalvedReduce2DY<ReduceOp, blockSize_y>(shared_array, &acc, op, sharedmem_x);
  if (threadIdx.y == 0) {
