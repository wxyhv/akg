...
isl::schedule Scop::GenIsl() {
  ...
  info_.mmu_info_.ComputeByPassC1();
  return schedule_tmp;
}
...
isl::schedule Scop::Transform(const isl::schedule &input_schedule) {
  ...
  auto final_schedule = mgr.Run(input_schedule, dsa_strategy);
...
Stmt GenHalide(ScopInfo &info, const isl::schedule &sch, bool used_for_tile_out_band) {
  ...
  TIMER_SHOW("NPUIslEmitter", std::string(info.mmu_info_.IsSpecGemm() ? "_specgemm" : ""));
