...
Stmt NPUIslEmitter::EmitGemmRangeInfoBackPropFilter(const Stmt &stmt) {
  ...
  return AttrStmt::make(range_map, PRAGMA_GEMM_C0, Expr(l0_range_idx), stmt);
}
...
Stmt NPUIslEmitter::InsertRealize(Stmt stmt, const isl::id &var, bool is_C0) {
  ...
  if (info_.mmu_info_.IsCBUF(var.get_name())) {
...
Stmt HoistC0write(ScopInfo &info, const Stmt &body, std::vector<Stmt> &l0write) {
  Stmt stmt = body;
  if (!l0write.empty()) {
    if (info.mmu_info_.IsGemm()) {
      auto f = HoistC0Write(info.user_config_.GetOriginBind(), l0write.back());
...
void NPUIslEmitter::ProcBypassC1(const ScopInfo &info) {
  if (0 == bypassC1_) {
    bypassC1_ = info.user_config_.GetByPathC1();
  }
}
...
void NPUIslEmitter::EmitReadAttrAtC1(std::vector<Stmt> &stmts, int i, Tensor &t) {
  ...
  if ((bypassC1_ == 2 && data_str == t->op->name) || (bypassC1_ == 1 && weight_str == t->op->name)) {
    is_filter_l1 = true;
  }
...
void NPUIslEmitter::EmitWriteAttr(const std::vector<IslIdSet> &write, std::vector<Stmt> &stmts, int i, bool is_C1) {
  for (const auto &id : write[i]) {
    if (is_C1 && info_.mmu_info_.IsCBUF(id.get_name())) continue;
    ...
    if (info_.mmu_info_.IsGemm() && !info_.mmu_info_.IsSpecGemm() && info_.mmu_info_.IsCBUF(id.get_name())) {
      stmts[i] = AttrStmt::make(make_zero(Int(32)), PRAGMA_MMU_C0WRITE, Expr(1), stmts[i]);
      mmu_c0write_.emplace_back(stmts[i]);
      stmts[i] = Evaluate::make(0);
    }
...
void NPUIslEmitter::RealizeOut() {
  ...
  for (const auto &j : realize_must_def_) {
    ...
    if (info_.MayWriteAfterRead(tensor_name)) {
      bool do_out = true;
      auto c_buf =
        info_.mmu_info_.IsSpecGemm() ? info_.mmu_info_.GetCName() : info_.mmu_info_.GetCName() + LOCAL_BUF;
      auto c_c0c = c_buf + LOCAL_C0C;
