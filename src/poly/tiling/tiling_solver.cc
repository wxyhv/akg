 ...
 *               Allocation exceed bound of memory tag local.BUF.
...
double TilingSolver::GetNewAllocRatioWhenRewriteFail(int64_t memory_bits) {
  ...
  auto last_adjust_ratio = global_attrs.GetFloatAttr(kBUFRatio, 1.0);
  ...
  if (actual_allocs != 0) {
    ...
    global_attrs.Set(kBUFRatio, air::make_const(Float(32), adjust_ratio));
...
void TilingSolver::CollectMemoryLimit() {
  ...
  // Init memory limit for each scope and reduce ratio of local.BUF if storage rewrite fails previously.
  ...
  for (auto i = 0; i < MEM_SCOPE_BULK; ++i) {
    ...
    if (i == NpuMemScope::MEM_SCOPE_BUFFER && error_scope == DOT_LOCAL_BUF) {
      this->mem_limit_[i] =
...
void InequalitySolver::InitTileAxis(TileLevel level) {
  ...
  for (auto axis : this->cand_.GetTileAxis()) {
    ...
    std::string var_name = level == LEVEC1 ? "T1_" : "T0_";
...
TileCandidate *InequalitySolver::Solve() {
  ...
  for (auto band = 0; band < tile_band_size; ++band) {
    ...
    for (auto i = static_cast<int>(tile_axes.size()) - 1; i >= 0; --i) {
      ...
      DetermineTileFactor(axis, LEVEC1, memory_constraints);
    }
    if (analyzer_.op_type_ != INST_OP) {
      for (auto i = static_cast<int>(tile_axes.size()) - 1; i >= 0; --i) {
        TileAxis *axis = tile_axes[i];
        DetermineTileFactor(axis, LEVEC0, memory_constraints);
      }
    }
  }
  return &cand_;
...
void InequalitySolver::DetermineTileFactor(TileAxis *axis, TileLevel level, const Array<Expr> &memory_constraints) {
  if (axis->is_pragma && level == LEVEC1) {
    return;
  }
  ...
  Expr to_tile = level == LEVEC1 ? l1_expr : l0_expr;
  TileAxis::Constraint cons = level == LEVEC1 ? axis->l1_constraints : axis->l0_constraints;
  ...
  if (to_tile.as<Variable>()) {
    ...
    if (is_static_shape) {
      ...
      param_info_.push_back(ParamInfo{"AttrStmt", Expr("[MemoryLimit_BUF]"), to_tile <= shape_range});
      ...
      param_info_.push_back(ParamInfo{"AttrStmt", Expr("[MemoryLimit_BUF]"), to_tile <= final_factor_expr});
...
Expr InequalitySolver::DetermineTileForDynamic(TileAxis *axis, const Expr &mem_constraint, const Expr &to_tile,
  ...
  if (analyzer_.op_type_ != CONV_OP && infer_bound_fail) {
    ...
    param_info_.push_back(ParamInfo{"AttrStmt", Expr("[MemoryLimit_BUF]"), to_tile <= new_mem_constraint});
  ...
  if (level == LEVEC1 && axis->forbid_iso) {
...
void InequalitySolver::AppendShapeLimitConstraint(TileAxis *axis, Expr to_tile) {
  if (axis->dyn_shape_limit == -1) {
    ...
    param_info_.push_back(
      ParamInfo{"AttrStmt", Expr("[MemoryLimit_BUF]"), axis->range_extent <= CastIntToExpr(axis->dyn_shape_limit)});
  }
}
...
int64_t InequalitySolver::DetermineTileForStatic(TileAxis *axis, const Expr &mem_constraint, const Expr &tile_range,
  ...
  if (!cons.cand_factor.empty()) {
    ...
    if (static_mem_constraint >= static_shape) {
      ...
      bool need_to_align = (final_factor > mod_value || !axis->HasAttr("INSTIZED"));
    ...
    if (analyzer_.scop_info_.user_config_.GetPragmaAnalyzeMulticore() && !analyzer_.is_dynamic_ &&
        analyzer_.op_type_ == INST_OP && analyzer_.scop_info_.user_config_.GetTarget() != TARGET_CUDA) {
...
void InequalitySolver::CalculateMemoryInBuffer(const TilingAnalyzer::BufferEntry *buf, TilingMemInfo *mem_info) {
  ...
  bool is_l0_buf = buf->scope > MEM_SCOPE_C1;
...
Array<Expr> InequalitySolver::CollectMemoryConstraints() {
  std::unordered_map<int, std::string> memory_map = {{1, BUF}, {2, C1}, {3, C0A}, {4, C0B}, {5, C0C}};
  auto mem_info = tiling_mem_info_.get();
...
TileCandidate *DynamicShapeSolver::Solve() {
  ...
  for (auto band = 0; band < tile_band_size; ++band) {
    tiling_band_ = band;
    AppendTileConstraintInIR(result, TileLevel::LEVEC1);
    if (analyzer_.op_type_ == GEMM_OP) {
      AppendTileConstraintInIR(result, TileLevel::LEVEC0);
    }
  }
  return result;
}
...
void DynamicShapeSolver::AppendTileConstraintInIR(TileCandidate *cand, TileLevel level) {
  auto Append = [this, level, cand](TileAxis *axis) {
    ...
    TileAxis::Constraint cons = level == LEVEC1 ? axis->l1_constraints : axis->l0_constraints;
    Expr tile_var = level == LEVEC1 ? cand->tile_val_[axis].tile_l1 : cand->tile_val_[axis].tile_l0;
...
TileCandidate *TraverseSolver::Solve() {
  ...
  for (auto band = 0; band < tile_band_size; ++band) {
    ...
    for (TileAxis *axis : cand_.GetTileAxis()) {
      std::unique_ptr<TileInfo> info(new (std::nothrow) TileInfo(axis, LEVEC1, band));
    ...
    if (analyzer_.op_type_ == GEMM_OP) {
      for (TileAxis *axis : cand_.GetTileAxis()) {
        std::unique_ptr<TileInfo> info(new (std::nothrow) TileInfo(axis, LEVEC0, band));
        CHECK(info) << "memory alloc fail";
        if (IsTilable(info.get())) {
          if (DoTiling(info.get())) break;
        }
...
bool TraverseSolver::IsTilable(TileInfo *info) {
  ...
  if (level == LEVEC1) {
    bool use_tile_min = (info->axis->forbid_iso && const_extent % cons.tile_mod_.as<IntImm>()->value != 0) ||
                        (cons.tile_min_.as<IntImm>()->value == MIN_TILE) || (axis->HasAttr("INSTIZED")) ||
...
bool TraverseSolver::MemoryVerify(TileLevel level, int band, int64_t *deviation) {
  ...
  for (int i = 0; i < MEM_SCOPE_BULK; ++i) {
    auto scope = static_cast<NpuMemScope>(i);
    ...
    if (level == LEVEC0 && i > MEM_SCOPE_BUFFER) {
  ...
  bool C1_valid = (expanded_size[MEM_SCOPE_C1] <= mem_limit_[MEM_SCOPE_C1]);
  bool BUF_valid = (expanded_size[MEM_SCOPE_BUFFER] <= mem_limit_[MEM_SCOPE_BUFFER]);
  bool C0A_valid = (expanded_size[MEM_SCOPE_C0A] <= mem_limit_[MEM_SCOPE_C0A]);
  bool C0B_valid = (expanded_size[MEM_SCOPE_C0B] <= mem_limit_[MEM_SCOPE_C0B]);
  bool C0C_valid = (expanded_size[MEM_SCOPE_C0C] <= mem_limit_[MEM_SCOPE_C0C]);
  ...
  if ((!cut_reduce && level == LEVEC1 && (!C1_valid || (!BUF_valid && analyzer_.op_type_ == INST_OP))) ||
      ((cut_reduce || level == LEVEC0) && (!C0A_valid || !C0B_valid || !C0C_valid))) {
    return false;
  }
  return true;
}
...
bool TraverseSolver::DoTiling(const TileInfo *info) {
  ...
  CHECK_GT(cons.tile_extent_.as<IntImm>()->value, 0) << "Static shape's C1 max factor should be positive integer";
  int64_t init = info->min_tile;
  int64_t dst = info->level == LEVEC1 ? cons.tile_extent_.as<IntImm>()->value : this->cand_.GetConstTileVal(axis).first;
...
void TraverseSolver::AppendConvPragma() {
  ...
  CHECK(M.defined());
  M = CanonicalSimplify((floordiv((M - 1 + MMU_UNIT), MMU_UNIT)) * MMU_UNIT);
  Expr mo = CanonicalSimplify(floordiv(M, MMU_UNIT));
  CreateSpecgemmTileAxis(mo, no, ko, false);
  this->cand_.SetBatchAxis(spec_tile_axis_);
  if (analyzer_.is_dynamic_) {
    cand_.InitTileAxis(LEVEC0);
  } else {
    for (TileAxis *axis : this->cand_.GetTileAxis()) {
      std::unique_ptr<TileInfo> info(new (std::nothrow) TileInfo(axis, LEVEC0, 0));
      CHECK(info) << "memory alloc fail";
      if (IsTilable(info.get())) {
...
void TraverseSolver::RestrainConvBackInputTileK(TileAxis *k_axis) const {
  ...
  Expr k_mod = k_h * k_w;
  k_axis->TileRestrainMod(k_mod, LEVEC0);
}
...
void TraverseSolver::CreateSpecgemmTileAxis(Expr mo, Expr no, Expr ko, bool cut_reduce) {
  ...
  TileAxis *mi_axis = GeneratePragmaAxes(MMU_UNIT, ATTR_CONV_M_INNER, true);
  TileAxis *ni_axis = GeneratePragmaAxes(MMU_UNIT, ATTR_CONV_N_INNER, true);
  TileAxis *ki_axis = GeneratePragmaAxes(MMU_UNIT, ATTR_CONV_K_INNER, true);
  if (cut_reduce) {
    mo_axis->TileRestrainEntire(LEVEC0);
    no_axis->TileRestrainEntire(LEVEC0);
  }
  ...
  std::unordered_map<std::string, std::vector<TileAxis *>> spec_map = {
    {C0A, {mo_axis, mi_axis, ko_axis, ki_axis}},
    {C0B, {no_axis, ni_axis, ko_axis, ki_axis}},
    {C0C, {mo_axis, mi_axis, no_axis, ni_axis}},
  };
