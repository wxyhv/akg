...
from akg.ops.poly_gpu import fused_conv2d_bp_bn_grad_manual, fused_conv2d_bp_bn_grad_auto

def conv2d_bp_bn_grad(data_sum, in_bn, head_active, in_active, layout):
...
def gen_data(shape, out_shape, dtype, out_dtype, layout):
    ...
    expect = conv2d_bp_bn_grad(data_sum, in_bn, head, in_active, layout)
    return head, data_sum, in_bn, in_active, output, expect

def test_fused_conv2d_bp_bn_grad(shape, out_shape, dtype, out_dtype, layout='NHWC', poly_sch=False):
    shape_list = [out_shape, shape, shape, shape]
    dtype_list = [out_dtype, dtype, dtype, dtype]
    op_attrs = [layout]
    if poly_sch:
        mod = utils.op_build(
            fused_conv2d_bp_bn_grad_auto,
            shape_list,
            dtype_list,
            op_attrs=op_attrs,
            attrs={
        ...
        mod = utils.op_build(fused_conv2d_bp_bn_grad_manual, shape_list, dtype_list, op_attrs=op_attrs)
