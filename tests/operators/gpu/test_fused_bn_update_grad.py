...
def test_fused_bn_update_grad(shape, dtype='float32', poly_sch=False):
    input = gen_data(shape, dtype)
    expect = compute_expect(input)
    input_shape = [shape, shape]
    input_dtype = [dtype, dtype]
    if poly_sch:
        mod = utils.op_build(fused_bn_update_grad_auto, input_shape, input_dtype, attrs={"target": "cuda"})
    else:
        mod = utils.op_build(fused_bn_update_grad_manual, input_shape, input_dtype)
