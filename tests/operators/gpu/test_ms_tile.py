...
import numpy as np
from akg.ops.poly_gpu import tile_manual, tile_auto
...
def test_ms_tile(shape, multiples, dtype, poly_sch=False):
    if poly_sch:
        mod = utils.op_build(tile_auto, [shape], [dtype], op_attrs=[multiples], attrs={"target": "cuda"})
    else:
        mod = utils.op_build(tile_manual, [shape], [dtype], op_attrs=[multiples])    
