...
from akg.ops.poly_gpu import fused_bn_relu_manual, fused_bn_relu_auto

def compute_fused_bn_relu(data, layout, inter_dtype, out_dtype):
    beta = data[0]
    gamma = data[1]
    bn_update = data[2]
    bn_reduce = data[3]
...
def gen_data(in_shape, in_dtype, inter_dtype, layout, out_dtype):
    ...
    expect = compute_fused_bn_relu(data, layout, inter_dtype, out_dtype)
    output = np.full(expect.shape, np.nan, out_dtype)

    return data, output, expect

def test_fused_bn_relu(in_shape, in_dtype='float16', layout='NHWC', out_dtype='float32', poly_sch=False):
    ...
    if poly_sch:
        ...
        mod = utils.op_build(fused_bn_relu_manual, input_shape_list, input_dtype_list, op_attrs=op_attrs)

    output = utils.mod_launch(mod, (data[0], data[1], data[2], data[3], data[4], output), expect=expect)
    ...
    data[0], data[1], data[2], data[3], data[4], expect = to_tvm_nd_array(
        [data[0], data[1], data[2], data[3], data[4], expect])
    gpu_profiling(mod, data[0], data[1], data[2], data[3], data[4], expect, 400)
