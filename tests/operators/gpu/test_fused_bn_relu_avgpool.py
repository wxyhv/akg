...
from akg.ops.poly_gpu import fused_bn_relu_avgpool_manual, fused_bn_relu_avgpool_auto

def compute_fused_bn_relu_avgpool(data, inter_dtype, layout, out_dtype):
    xi_conv2d1 = data[0]
    beta = data[1]
    gamma = data[2]
    bn_update = data[3]
...
def gen_data(in_shape, in_dtype, inter_dtype, layout, out_dtype):
    ...
    expect = compute_fused_bn_relu_avgpool(data, inter_dtype, layout, out_dtype)
    output = np.full(expect.shape, np.nan, out_dtype)

    return data, output, expect

def test_fused_bn_relu_avgpool(in_shape, in_dtype='float16', layout='NHWC', out_dtype='float16', poly_sch=False):
    ...
    if poly_sch:
        mod = utils.op_build(fused_bn_relu_avgpool_auto, input_shape_list, input_dtype_list,
                             op_attrs=op_attrs, attrs={"target": "cuda"})
    else:
        mod = utils.op_build(fused_bn_relu_avgpool_manual, input_shape_list, input_dtype_list, op_attrs=op_attrs)

    output = utils.mod_launch(mod, (data[0], data[1], data[2], data[3], data[4], data[5], output), expect=expect)
    ...
    data[0], data[1], data[2], data[3], data[4], data[5], expect = to_tvm_nd_array(
        [data[0], data[1], data[2], data[3], data[4], data[5], expect])
    gpu_profiling(mod, data[0], data[1], data[2], data[3], data[4], data[5], expect, 400)
