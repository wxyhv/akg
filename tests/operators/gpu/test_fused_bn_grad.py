...
from akg.ops.poly_gpu import fused_bn_grad_manual, fused_bn_grad_auto
from test_fused_pattern_grad import bn_beta_grad_np, bn_gamma_grad_np


def bn_grad(head, data_sum, in_bn, layout):
...
def gen_data(shape, out_shape, dtype, out_dtype, layout):
    ...
    expect = bn_grad(head, data_sum, in_bn, layout)
    return head, data_sum, in_bn, output, expect

def test_fused_bn_grad(shape, out_shape, dtype, out_dtype, layout='NHWC', poly_sch=False):
    shape_list = [shape, out_shape, shape]
    dtype_list = [dtype, out_dtype, dtype]
    op_attrs = [layout]
    if poly_sch:
        mod = utils.op_build(fused_bn_grad_auto, shape_list, dtype_list, op_attrs=op_attrs, attrs={"target": "cuda"})
    else:
        mod = utils.op_build(fused_bn_grad_manual, shape_list, dtype_list, op_attrs=op_attrs)
