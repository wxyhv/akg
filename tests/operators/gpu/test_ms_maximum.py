...
import numpy as np
from akg.ops.poly_gpu import maximum_manual, maximum_auto
...
def test_ms_maximum(shape1, shape2, dtype, poly_sch=False):
    if poly_sch:
        mod = utils.op_build(maximum_auto, (shape1, shape2), (dtype, dtype), attrs={"target": "cuda"})    
    else:
        mod = utils.op_build(maximum_manual, (shape1, shape2), (dtype, dtype))    
